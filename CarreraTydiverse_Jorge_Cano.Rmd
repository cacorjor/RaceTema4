...
```{r}
threshold_value <- cran %>%
  summarise(threshold = quantile(size, probs = 0.99)) %>%
  pull(threshold)


top_counts <- cran %>%
  filter(size > threshold_value)
...
top_counts_sorted <- cran %>%
  ...
  summarise(threshold = quantile(size, probs = 0.99)) %>%
  pull(threshold) -> threshold_value  
...
cran %>%
  group_by(country) %>%  
  summarise(total_traffic = sum(size)) %>%  
  arrange(desc(total_traffic)) %>%  
  mutate(cum_traffic = cumsum(total_traffic) / sum(total_traffic)) %>%  
  filter(cum_traffic <= 0.9) %>%  
  ggplot(aes(x = country, y = total_traffic)) +  
  geom_point() +
  
  labs(x = "Country", y = "Trafico Generado (Bytes)", title = "Pises con el 90% Download Traffic")  
